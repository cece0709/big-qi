...
function Challenge({puzzle,onComplete}:{puzzle:Puzzle;onComplete:(id:string)=>void}){
 ...
 function reset(){setHistory([{board:puzzle.board,last:undefined}]);setSelected(null);setDone(false);setHint(false);setAnswer(false);setCursor(0);setMessage('已恢复题面。红方先行，试着完成本关目标。');}
 function cell(x:number,y:number){
  ...
  if(result.complete){setDone(true);onComplete(puzzle.id);setMessage('练习完成！'+result.message);}
 }
...
export default function PuzzlesPage(){
 const [active,setActive]=useState(puzzles[0]),[solved,setSolved]=useState<string[]>([]),[category,setCategory]=useState('全部'),[query,setQuery]=useState(''),[tab,setTab]=useState<'puzzles'|'techniques'>('puzzles');
 ...
 function choose(p:Puzzle){setActive(p);setAttempt(n=>n+1);requestAnimationFrame(()=>workspace.current?.scrollIntoView({behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'start'}));}
 ...
 <section className="puzzle-library" aria-label="残局与技巧题库"><div className="library-heading"><h2>棋谱小藏</h2><span>本次已练习 {solved.length} / {puzzles.length}</span></div>
