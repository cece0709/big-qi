...
export function analyze(position:string|GameState,budget=1000,maxDepth=3):Advice{
 const g=typeof position==='string'?new Chess(position):replay(position),deadline=Date.now()+budget,side=g.turn(),escape=g.isCheck();
 const ended=outcome(g);if(ended)return {title:ended,why:'本局已经结束。',purpose:'可以复盘或重新开始。',idiom:'',depth:0};
 // Prefer fresh positions only when evaluation ties; never sacrifice a necessary drawing move.
 const seen=new Map<string,number>(),key=(fen:string)=>fen.split(' ').slice(0,4).join(' ');
 if(typeof position!=='string'){const past=new Chess(position.start);seen.set(key(past.fen()),1);for(const m of position.line){past.move(m);const k=key(past.fen());seen.set(k,(seen.get(k)||0)+1);}}
 const options=g.moves({verbose:true}).map(move=>{g.move(move);const repeats=seen.get(key(g.fen()))||0;g.undo();return {move,repeats};}).sort((a,b)=>a.repeats-b.repeats).map(x=>x.move);let chosen=options[0],depthDone=0;
 ...
 for(let depth=1;depth<=maxDepth;depth++){try{let top=-Infinity,candidate=chosen;for(const m of options){g.move(m);let v;try{v=-search(depth-1,-Infinity,-top,1);}finally{g.undo();}if(v>top){top=v;candidate=m;}}chosen=candidate;depthDone=depth;if(top>99000)break;}catch{break;}}
 if(!chosen)return {title:outcome(g)||'没有可走的棋',why:'当前没有合法走法。',purpose:'可以重新开始或返回摆局。',idiom:'',depth:depthDone};
 const move={from:chosen.from,to:chosen.to,promotion:chosen.promotion};const applied=g.move(move),checked=applied.san.includes('+');
 return {move,title:label(applied),why:explainMove(applied),purpose:applied.san.endsWith('#')?'本方完成将死。':escape?'先化解对王的威胁，再考虑进攻。':applied.captured?'争取子力收益；仍须观察对手是否能回吃。':checked?'迫使对方先应将，争取主动。':`${sideName(side)}选择了当前有限搜索中的推荐走法；请继续观察双方威胁，不保证全局最佳。`,idiom:applied.san.endsWith('#')?'一锤定音':escape?'转危为安':applied.captured?'权衡利弊':'稳扎稳打',depth:depthDone};
}
