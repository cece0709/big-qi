...
export default function App(){
 ...
 return <><AchievementToast/><div hidden={international}><ChineseApp active={!international} onSwitch={()=>switchTo(true)} sound={sound} setSound={setSound}/></div>{visited&&<div hidden={!international}><InternationalApp active={international} onSwitch={()=>switchTo(false)} sound={sound} setSound={setSound}/></div>}</>;
}
import DifficultyPicker from './components/DifficultyPicker';
import MoveFeedback from './components/MoveFeedback';
import {ProgressPanel,AchievementToast,useProgress} from './components/Progress';
import {readDifficulty,saveDifficulty,type Difficulty} from './game/difficulty';
import {readProgress,recordMove,finishGame,reopenGame} from './game/progress';
...
function ChineseApp({active,onSwitch,sound,setSound}:{active:boolean;onSwitch:()=>void;sound:boolean;setSound:(v:boolean)=>void}){const [page,setPage]=useState<Page>(navigation.some(n=>n.id===location.hash.slice(1))?location.hash.slice(1) as Page:'home');const [menu,setMenu]=useState(false);const [mode,setMode]=useState<'ai'|'local'>('ai');const [steps,setSteps]=useState<RecordStep[]>([{board:initial(),turn:'red',label:'开局 · 红方先行'}]);const [selected,setSelected]=useState<string|null>(null);const [message,setMessage]=useState('点一点红色棋子，看看它能去哪里。绿色小点就是可以走的位置。');const [ended,setEnded]=useState('');const [review,setReview]=useState(0);const [kind,setKind]=useState<Kind>('r');const [completed,setCompleted]=useState<Kind[]>(()=>readProgress().pieces);const [puzzle,setPuzzle]=useState<number|null>(null);const [solved,setSolved]=useState<number[]>([]);
const progress=useProgress();
const [difficulty,setDifficulty]=useState<Difficulty>(readDifficulty),[feedback,setFeedback]=useState(''),[pending,setPending]=useState<Move|null>(null);
...
function reset(nextMode=mode,puzzleIndex:number|null=null){gameId.current=crypto.randomUUID();setFeedback('');setPending(null);setMode(nextMode);setPuzzle(puzzleIndex);setSteps([{board:puzzleIndex===null?initial():puzzles[puzzleIndex].board,turn:'red',label:'开局 · 红方先行'}]);setSelected(null);setEnded('');setMessage(puzzleIndex===null?'新的一局，不着急。点击红色棋子开始吧。':puzzles[puzzleIndex].desc);}
function commit(m:Move){setFeedback('');setPending(null);recordMove();const p=board.find(p=>p.id===m.id)!;const target=board.find(p=>p.x===m.x&&p.y===m.y);const next=apply(board,m);const nextTurn=other(turn);const options=moves(next,nextTurn);let finish='';if(!options.length)finish=(turn==='red'?'红方':'黑方')+'获胜'+(inCheck(next,nextTurn)?'，将杀！':'，对方无棋可走。');setSteps(s=>[...s,{board:next,turn:nextTurn,last:m,label:`${turn==='red'?'红':'黑'}${names[p.side][p.kind]}：${p.x+1}路${p.y+1}行 → ${m.x+1}路${m.y+1}行${target?'，吃'+names[target.side][target.kind]:''}`}]);setSelected(null);setEnded(finish);if(finish&&puzzle===null)finishGame(gameId.current,turn==='red'?'win':'loss',mode==='ai');setMessage(finish||(inCheck(next,nextTurn)?'将军！帅（将）正受到攻击，下一步必须解除威胁。':target?'吃到棋子啦。也别忘记保护自己的棋子。':'走得不错，每一步都在积累棋感。'));
...
useEffect(()=>{if(!(active&&mode==='ai'&&turn==='black'&&!ended&&puzzle===null&&(page==='play'||page==='home')))return;let alive=true;const worker=new Worker(new URL('./game/opponent.worker.ts',import.meta.url),{type:'module'});worker.onmessage=({data})=>{if(alive&&data.move)commit(data.move);else if(alive)setMessage('陪练暂时没有回应，可以重开或换一种难度。');};worker.onerror=()=>{if(alive)setMessage('陪练暂时没有回应，可以重开或换一种难度。');};worker.postMessage({board,side:'black',difficulty});return()=>{alive=false;worker.terminate();};},[steps,mode,ended,page,puzzle,active,difficulty]);
function cell(x:number,y:number){if(ended){setMessage('本局已结束，可以重新开始或前往复盘。');return;}if(mode==='ai'&&turn==='black'&&puzzle===null){setMessage('弈见正在想下一步，请稍等。');return;}const clicked=board.find(p=>p.x===x&&p.y===y);if(clicked?.side===turn){setSelected(clicked.id===selected?null:clicked.id);setMessage(lessons[clicked.kind].rule);return;}const p=board.find(p=>p.id===selected);if(!p){setMessage(`先选择一枚${turn==='red'?'红':'黑'}方棋子，再点击想去的位置。`);return;}const error=reason(board,p,x,y);if(error){setMessage(error);setFeedback(error);setPending(null);return;}const m={id:p.id,x,y},warning=riskyMove(board,m);if(warning){setFeedback(warning);setPending(m);return;}commit(m);}
...
function undo(){reopenGame(gameId.current);setFeedback('');setPending(null);const count=mode==='ai'&&turn==='red'&&puzzle===null?2:1;setSteps(s=>s.slice(0,Math.max(1,s.length-count)));setSelected(null);setEnded('');setMessage('退回一步，再想想也很好。');}
...
return <div className="app"><aside className={'sidebar '+(menu?'open':'')}><Brand/><div className="side-caption">一方棋盘，一段静好时光</div><nav>{navigation.map(n=><button className={page===n.id?'active':''} key={n.id} onClick={()=>go(n.id)}><n.icon size={19}/><span>{n.label}</span>{page===n.id&&<i/>}</button>)}</nav><div className="side-bottom"><div className="quote-mark">“</div><p>棋不必走得快，<br/>每一步，都算成长。</p><span>慢慢来，弈见陪着你</span><div className="side-divider"/><div className="guest" role="button" tabIndex={0} onClick={()=>{go('home');setTimeout(()=>document.querySelector('.progress-panel')?.scrollIntoView({behavior:'smooth'}),0);}} onKeyDown={e=>{if(e.key==='Enter')go('home');}}><span className="avatar">初</span><div>初见棋友<small>已认识 {progress.pieces.length} 种棋子</small></div><Leaf size={17}/></div></div></aside><main><header><button className="mobile-menu" onClick={()=>setMenu(!menu)} aria-label="打开导航">{menu?<X/>:<Menu/>}</button><div className="breadcrumb">弈见 <span>/</span> {navigation.find(n=>n.id===page)?.label||'棋院首页'}</div><div className="header-right"><span><i/> 慢慢学，步步有收获</span><button onClick={()=>{setSoundEnabled(!sound);setSound(!sound);if(!sound)chessSound('place')}} aria-pressed={sound} title={sound?'关闭落子音效':'开启落子音效'} aria-label={sound?'关闭落子音效':'开启落子音效'} className={sound?'sound-on':''}>{sound?<Volume2 size={18}/>:<VolumeX size={18}/>}</button><span className="avatar small">初</span></div></header>
<div className="content"><div className="variant-bar"><span><b>弈</b> 中国象棋 <small>XIANGQI</small></span><button onClick={onSwitch}>⇄ 切换到国际象棋</button></div>{(page==='home'||page==='play')&&<DifficultyPicker value={difficulty} started={steps.length>1} onChange={v=>{saveDifficulty(v);setDifficulty(v);reset();}}/>}{feedback&&(page==='home'||page==='play')&&<MoveFeedback message={feedback} onRetry={()=>{setFeedback('');setPending(null);}} onShow={()=>{setFeedback('');setPending(null);hint();}} onContinue={pending?()=>commit(pending):undefined}/>}{page==='home'&&<ProgressPanel/>}{page==='home'&&<HomePage go={go} reset={reset} board={board} selected={selected} cell={cell} message={message} setKind={setKind}/>} 
