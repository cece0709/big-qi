...
export function search(board:Piece[],side:Side,budget=1800){const deadline=Date.now()+budget;const root=ordered(board,side);let best=root[0] as Move|undefined,completed=0;
 ...
 for(let depth=1;depth<=3;depth++){try{let score=-Infinity,candidate=best;for(const m of root){const n=apply(board,m);const v=-negamax(n,other(side),depth-1,-Infinity,-score,1);if(v>score){score=v;candidate=m;}}best=candidate;completed=depth;if(best)root.sort((a,b)=>a===best?-1:b===best?1:0);}catch{break;}}
